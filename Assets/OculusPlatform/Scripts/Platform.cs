...
namespace Oculus.Platform
{
  ...
  public static partial class Leaderboards
  {
    ...
    public static Request WriteEntry(string leaderboardName, long score, byte[] extraData = null, bool forceUpdate = false)
    {
      if (Core.IsInitialized())
      {
        return new Request(CAPI.ovr_Leaderboard_WriteEntry(leaderboardName, score, extraData, (uint)(extraData != null ? extraData.Length : 0), forceUpdate));
      }
  ...
  public static partial class Matchmaking
  {
    ...
    /// \param subscribeToUpdates If true, sends a message with type ??? when the room data changes, such as when users join or leave.
    ...
    /// \param pool The matchmaking pool to use, which is defined for the app.
    /// \param maxUsers Overrides the Max Users value, which is configured in pool settings of the Developer Dashboard.
    /// \param subscribeToUpdates If true, sends a message with type ??? when room data changes, such as when users join or leave.
    ///
    ...
    /// returns a ??? message when a match is found. Call Matchmaking.JoinRoom() on
    /// the returned room. The response contains useful information to display to
    /// the user to set expectations for how long it will take to get a match.
    ///
    ...
    /// \param roomID Returned either from ??? or from Matchmaking.CreateRoom().
    ...
    /// available matchmaking room. ??? gets enqueued when a match is found.
    ///
    ...
    /// Matchmaking.Browse().
    /// \param roomID ID of a room previously returned from ??? or Matchmaking.Browse().
    /// \param subscribeToUpdates If true, sends a message with type ??? when room data changes, such as when users join or leave.
    ///
